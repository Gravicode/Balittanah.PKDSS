...
public class WL_WN_CalibrationPanel extends JPanel {
	...
	public WL_WN_CalibrationPanel(String runTime, String resolution, String opticalSettings, String apodizationIndex, String zeroPaddingMultiplier, String tabPrefix) {
		super();
	...
	public void initialize() {
		...
		frmWL_Calib.setMinimumSize(new Dimension((int)(p2Constants.SUB_PANELS_DIMENTION * 1.2), (int)(p2Constants.SUB_PANELS_DIMENTION)));
		frmWL_Calib.setMaximumSize(new Dimension((int)(p2Constants.SUB_PANELS_DIMENTION * 1.2), (int)(p2Constants.SUB_PANELS_DIMENTION)));
		frmWL_Calib.setPreferredSize(new Dimension((int)(p2Constants.SUB_PANELS_DIMENTION * 1.2), (int)(p2Constants.SUB_PANELS_DIMENTION)));
//		frmWL_Calib.setLocation((int)(UserInterface.screenSize.getWidth()/2 - p2Constants.SUB_PANELS_DIMENTION / 2), (int)(UserInterface.screenSize.getHeight()/2 - p2Constants.SUB_PANELS_DIMENTION / 2));
		...
		rdbtn_Self_Calib.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(tabPrefix.equals(p2Constants.SpecPrefix))
				{
		...
		if(tabPrefix.equals(p2Constants.SpecPrefix))
		{
			...
			cmb_StandardCalibrator_WL_Calib.setMaximumSize(new Dimension(
					p2Constants.MAX_WIDTH_OF_FIELD * 2, cmb_StandardCalibrator_WL_Calib.getPreferredSize().height));
			...
			//UserInterface.displayStandardCalibrators();
			//cmb_StandardCalibrator_WL_Calib.setSelectedIndex(0);
			...
			btn_LoadStandardCalibrator_WL_Calib.setMaximumSize(new Dimension(p2Constants.MAX_WIDTH_OF_FIELD, btn_LoadStandardCalibrator_WL_Calib.getPreferredSize().height));
		...
		btn_Proceed_WL_Calib.setMaximumSize(new Dimension((int)(p2Constants.MAX_WIDTH_OF_FIELD), btn_Proceed_WL_Calib.getPreferredSize().height));
		...
		btn_Proceed_WL_Calib.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(null != runTime && "" != runTime && null != resolution && "" != resolution
						&& null != opticalSettings && "" != opticalSettings && null != tabPrefix && "" != tabPrefix)
				{
					if(rdbtn_RM_Corr.isSelected())
					{
		...
		btn_Close_WL_Calib.setMaximumSize(new Dimension((int)(p2Constants.MAX_WIDTH_OF_FIELD), btn_Close_WL_Calib.getPreferredSize().height));
	...
	private void wavelengthCalibrationBG_Sequence()
	{
		...
		try
		{
			...
			try{
				...
				UserInterface.applicationManager.setOpticalSettings(opticalSettings, tabPrefix);
				logger.info("Setting Optical Settings finished");


				UserInterface.applicationManager.setOpticalSettings(opticalSettings, tabPrefix);

				if(p2Constants.STANDARD_CALIBRATOR_DEFAULT_CHOISE.equals(cmb_StandardCalibrator_WL_Calib.getSelectedItem().toString())
...
//			UserInterface.progressPar.setValue(0);
			...
			p2AppManagerStatus status = UserInterface.applicationManager.wavelengthCalibrationBG(runTime, 
					apodizationIndex,
...
//					UserInterface.colorLabel.setBackground(Color.red);
				...
				VariableHelper.setMessage("Background measurement started. Please wait...");
...
//			UserInterface.colorLabel.setBackground(Color.red);
	...
	private void wavelengthCalibrationSequence()
	{
		...
		try
		{
			...
			try{
				UserInterface.applicationManager.setSettings(resolution, "false");
...
//			UserInterface.progressPar.setValue(0);
			...
			p2AppManagerStatus status = UserInterface.applicationManager.wavelengthCalibration(runTime, 
					cmb_StandardCalibrator_WL_Calib.getSelectedItem().toString(),
					apodizationIndex,
					zeroPaddingMultiplier);
...
//					UserInterface.colorLabel.setBackground(Color.red);
				...
				VariableHelper.setMessage("Wavelength calibration started. Please wait...");
	...
	private void selfCorrCalibSequence()
	{
		...
		try{
			...
			try
			{
				logger.info("setSettings function (reg file rewrite) started");
				//true is for reloading t.reg
				UserInterface.applicationManager.setSettings(p2Constants.TWO_POINTS_CORR_CALIB_FOLDER_NAME, "true");
				logger.info("setSettings function (reg file rewrite) finished");

				SpectroscopyPanel.lastResolutionSelected = p2Constants.TWO_POINTS_CORR_CALIB_FOLDER_NAME;
				SpectroscopyPanel.lastZeroPaddingSelected = zeroPaddingMultiplier;
				InterSpecPanel.lastResolutionSelected = p2Constants.TWO_POINTS_CORR_CALIB_FOLDER_NAME;
...
//			UserInterface.progressPar.setValue(0);
			...
			p2AppManagerStatus status = UserInterface.applicationManager.runCalibCorr(runTime, apodizationIndex, zeroPaddingMultiplier);
...
//					UserInterface.colorLabel.setBackground(Color.red);
				...
				VariableHelper.setMessage("Calibration started. Please wait...");
		...
		if (arg1 instanceof p2AppManagerNotification) {
			...
			case 25: // Interferogram gain and offset correction parameters calculation
				if (resp.getStatus() == 0) {
					...
					VariableHelper.setMessage("Calibration is completed successfully!");
...
//					UserInterface.progressPar.setValue(100);
					...
					JOptionPane.showMessageDialog(frmWL_Calib,
							"Calibration results are successfully saved!",
...
//					UserInterface.progressPar.setValue(100);
			...
			case 30: // BG measurement for Wavelength calibration
				if (resp.getStatus() == 0) {
					...
					VariableHelper.setMessage("BG measurement is completed successfully!");
//					UserInterface.colorLabel.setForeground(Color.green.darker());
//					UserInterface.colorLabel.setBackground(Color.green.darker());
